...
from bo.synthetic_test_functions.synthetic_test_functions import ConstrainedBraninNew, \
    ConstrainedFunc3, ConstrainedFunc3Redundant, PressureVessel, SpeedReducer
...
class TestDecoupledKG(BotorchTestCase):
    ...
    def test_speed_reducer(self):
        ...
        # c5, c6 and c8 are active at the published optimum, so their values sit at 0 up to
        # rounding; feasibility is asserted with a tolerance rather than a strict c <= 0.
        for constraint_idx in range(1, 12):
            value = function.evaluate_task(normalized_best_recommended_point, constraint_idx).item()
            self.assertLessEqual(value, 1e-3, f"constraint {constraint_idx} infeasible: {value}")
        self.assertAllClose(expected_best_fval, actual_full_vector[:, 0].item(), atol=1e-3)
        self.assertAllClose(actual_constraint1_value.item(), actual_full_vector[:, 1].item(), atol=1e-4)
        self.assertAllClose(actual_constraint2_value.item(), actual_full_vector[:, 2].item(), atol=1e-4)
        self.assertAllClose(actual_constraint3_value.item(), actual_full_vector[:, 3].item(), atol=1e-4)
