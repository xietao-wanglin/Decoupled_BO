...
By default the benchmark is **single-threaded**: every CPU thread pool (torch
intra-op and inter-op, OpenMP/MKL/OpenBLAS/NumExpr/vecLib and numba) is pinned
to one thread, which gives more stable timings, a clearer relationship with the
training size, easier reproducibility across machines and no ambiguity about
parallel execution. Use ``--threads N`` to measure with N threads instead.
...
# Thread-pool environment variables pinned before torch/numba are imported. The
# numba entry matters: the KGCB epigraph sweep used by every dcKG acquisition is
# a numba.prange region (refactored_acquisition_functions.py:142-161), so it
# would otherwise run multi-threaded regardless of the torch thread count.
...
def parse_args():
    ...
    parser.add_argument("--threads", type=int, default=1,
                        help="Threads for every CPU pool (torch intra-op and inter-op, BLAS, "
                             "numba). Default 1: single-threaded timings are more stable, scale "
                             "more clearly with the training size and reproduce across machines.")
...
def pin_thread_pools(threads):
    """Pin every CPU thread pool to ``threads``.

    Must be called before torch and numba are imported: the OpenMP/MKL/BLAS and
    numba pools read their environment variables at library load time, and
    torch.set_num_interop_threads only takes effect before any parallel work has
    started. All imports of torch and of the ``bo`` package in this module are
    function-local so that this runs first.
    """
    for variable in THREAD_ENV_VARS:
        os.environ[variable] = str(threads)


def apply_torch_threads(threads):
    """Runtime half of pin_thread_pools: the torch and numba thread counts."""
    import torch
    torch.set_num_threads(threads)
    try:
        torch.set_num_interop_threads(threads)
    except RuntimeError:
        # Already initialised (e.g. main() called twice in one process); the
        # environment variables above still cap the pools.
        pass
    try:
        import numba
...
def write_metadata(path, args, threads, interop_threads, torch_version):
    metadata = {
        "benchmark": BENCHMARK,
        "device": DEVICE_NAME,
        "dtype": "torch.float64",
        ...
        "single_threaded": threads == 1 and interop_threads == 1,
        "thread_pool_env": {variable: os.environ.get(variable) for variable in THREAD_ENV_VARS},
        "threading_note": (
            "Every CPU thread pool is pinned to the same count: the torch intra-op and inter-op "
            "pools, the OpenMP/MKL/OpenBLAS/NumExpr/vecLib pools (pinned via environment "
            "variables before torch is imported) and the numba pool (the KGCB epigraph sweep "
            "used by every dcKG acquisition is a numba.prange region). A single-threaded run "
            "gives more stable timings, a clearer relationship with the training size, easier "
            "reproducibility across machines and no ambiguity about parallel execution."
        ),
...
def main(args):
    # Before the first torch / bo / numba import in this process.
    pin_thread_pools(args.threads)
    ...
    threads, interop_threads = apply_torch_threads(args.threads)
    if threads != args.threads or interop_threads != args.threads:
        logging.warning(f"Requested {args.threads} thread(s) but torch reports "
                        f"{threads} intra-op / {interop_threads} inter-op.")

    logging.info(f"Mystery timing benchmark on {DEVICE}, {threads} intra-op / "
                 f"{interop_threads} inter-op thread(s), torch {torch.__version__}, "
    ...
    write_metadata(os.path.join(out, "mystery_timing_metadata.json"), args, threads,
                   interop_threads, torch.__version__)
